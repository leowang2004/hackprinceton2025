...
const USE_DEDALUS_BRIDGE = process.env.USE_DEDALUS_BRIDGE === "true";
const DEDALUS_BRIDGE_URL = process.env.DEDALUS_BRIDGE_URL || "http://localhost:8000";
const DEDALUS_BRIDGE_BASE = DEDALUS_BRIDGE_URL.replace(/\/$/, "");
const CHAT_CHUNK_SIZE =
  Number.parseInt(process.env.CHAT_RESPONSE_CHUNK_SIZE || "", 10) || 350;
const CHAT_CHUNK_DELAY_MS =
  Number.parseInt(process.env.CHAT_RESPONSE_DELAY_MS || "", 10) || 1100;
...
console.log(`[CONFIG] Dedalus bridge: ${USE_DEDALUS_BRIDGE ? `enabled (${DEDALUS_BRIDGE_URL})` : 'disabled'}`);
...
async function callDedalusBridge(endpoint, payload) {
  if (!USE_DEDALUS_BRIDGE) {
    throw new Error("Dedalus bridge is disabled");
  }

  const url = `${DEDALUS_BRIDGE_BASE}${endpoint}`;
  try {
    ...
    console.error(`[DEDALUS] Bridge call to ${endpoint} failed:`, error.message);
    throw error;
  }
}

function formatFinalMessage(message) {
  if (!message) return "";
  ...
  // Replace bullet-like Unicode characters within text
  text = text.replace(/[•▪♦◦]/g, "");
...
function splitMessageIntoChunks(message, maxLength) {
  ...
  if (itemMatchIndex !== -1) {
    ...
    let match;
    while ((match = itemRegex.exec(normalized.slice(itemMatchIndex))) !== null) {
...
async function generateSQLFromNaturalLanguage(naturalLanguageQuery) {
  ...
  try {
    ...
    // Try Dedalus bridge before OpenAI fallback
    if (USE_DEDALUS_BRIDGE) {
      try {
        const bridgeResponse = await callDedalusBridge("/generate-sql", {
          question: naturalLanguageQuery,
          schema: schemaContext,
        });

        if (bridgeResponse?.sql) {
          console.log("[DEDALUS] SQL generated via bridge");
          return bridgeResponse.sql;
        }
      } catch (error) {
        console.warn("[DEDALUS] Bridge SQL generation failed:", error.message);
      }
    }
...
/**
 * Generate shopping recommendations based on user's transaction history
 * @param {string} category - Optional category filter (shoes, electronics, food, etc.)
 * @returns {Promise<string>} - Formatted recommendations
 */
async function generateShoppingRecommendations(category = null) {
  if (!process.env.OPENAI_API_KEY) {
    return "I'd love to help you decide, but I need access to your transaction history. Please set up OpenAI API key.";
  }

  try {
    // Get user's transaction history from Snowflake or local data
    let purchaseHistory = [];
    let topMerchants = [];
    ...
    if (USE_SNOWFLAKE) {
      try {
        ...
        try {
          // Get top products and merchants
          const historyQuery = `
          SELECT 
            merchant_name,
            product_name,
          ...
          const results = await executeQuery(connection, historyQuery);

          purchaseHistory = results.map(row => ({
            merchant: row.MERCHANT_NAME || row.merchant_name,
            product: row.PRODUCT_NAME || row.product_name,
            amount: parseFloat(row.PRODUCT_TOTAL || row.product_total || 0),
            date: row.DATETIME || row.datetime
          }));

          // Get merchant spending (reuse same connection)
          const merchantQuery = `
          SELECT 
            merchant_name,
          ...
          const merchantResults = await executeQuery(connection, merchantQuery);

          topMerchants = merchantResults.map(row => ({
            name: row.MERCHANT_NAME || row.merchant_name,
            count: row.TRANSACTION_COUNT || row.transaction_count,
            total: parseFloat(row.TOTAL_SPENT || row.total_spent || 0)
          }));
        } finally {
          connection.destroy();
        }
      } catch (snowflakeErr) {
        console.error("[ERROR] Failed to fetch from Snowflake:", snowflakeErr.message);
        // Fallback to local data
        const items = getAllLineItems();
        purchaseHistory = items.slice(0, 50).map(item => ({
          merchant: item.merchant_name,
          product: item.name,
          amount: item.total,
          date: item.datetime
        }));
        topMerchants = topProducts({ limit: 10 }).map(p => ({
          name: p.name,
          count: p.qty,
          total: p.spend
        }));
    ...
    const merchants = new Set(purchaseHistory.map(h => h.merchant));
    const avgSpend = purchaseHistory.length > 0 
      ? purchaseHistory.reduce((sum, h) => sum + h.amount, 0) / purchaseHistory.length 
      : 0;

    spendingPatterns = {
      favoriteMerchants: Array.from(merchants).slice(0, 5),
      averagePurchaseAmount: avgSpend,
      totalPurchases: purchaseHistory.length,
      topMerchants: topMerchants.slice(0, 5)
    };

    // Use Dedalus bridge if available
    if (USE_DEDALUS_BRIDGE) {
      try {
        const bridgeResponse = await callDedalusBridge("/shopping-recommendations", {
          category,
          purchase_history: purchaseHistory.slice(0, 50),
          top_merchants: topMerchants,
          average_purchase_amount: avgSpend,
          favorite_merchants: spendingPatterns.favoriteMerchants,
          total_purchases: purchaseHistory.length,
        });

        if (bridgeResponse?.recommendations) {
          return bridgeResponse.recommendations;
        }

        if (bridgeResponse?.answer) {
          return bridgeResponse.answer;
        }
      } catch (error) {
        console.warn("[DEDALUS] Bridge shopping recommendations failed:", error.message);
      }
    }

    // Use ChatGPT to generate recommendations
    const prompt = `You are a helpful shopping assistant. Based on a customer's purchase history, generate 3 realistic product recommendations.

Customer's Purchase History:
${JSON.stringify(purchaseHistory.slice(0, 20), null, 2)}

Spending Patterns:
- Favorite merchants: ${spendingPatterns.favoriteMerchants.join(', ')}
- Average purchase amount: $${avgSpend.toFixed(2)}
- Total recent purchases: ${purchaseHistory.length}
- Top merchants by spending: ${topMerchants.map(m => `${m.name} ($${m.total.toFixed(2)})`).join(', ')}

${category ? `Customer is interested in: ${category}` : 'Customer wants shopping recommendations'}

Generate 3 fake but realistic product recommendations that:
1. Match the customer's spending patterns and preferences based on their history
2. Are appropriate for the category they mentioned (if any)
3. Include realistic product names, prices (based on their average spending), and brief descriptions
4. Are from merchants they've shopped at before (when possible)
5. Include a brief reason why each item might appeal to them based on their purchase history

Format as a friendly, conversational response. For each recommendation, include:
- Product name
- Price (realistic based on their spending patterns)
- Merchant/store (preferably one they've used)
- Brief description
- Why it might appeal to them

Make it sound natural and helpful, like a friend making recommendations.`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a helpful shopping assistant that makes personalized product recommendations based on purchase history. Be friendly, conversational, and specific."
        },
        {
          role: "user",
          content: prompt
    ...
    console.log("[SHOPPING] Generated recommendations based on purchase history");
    
    return recommendations;
  } catch (error) {
    console.error("[ERROR] Failed to generate shopping recommendations:", error.message);
    return "I'd love to help you decide, but I'm having trouble analyzing your purchase history right now. Please try again later.";
  }
}

function analyzeRecentSimilarPurchases({ category = null, keywords = [], lookbackDays = 365 } = {}) {
  const keywordMap = {
    laptop: ["laptop", "macbook", "notebook", "chromebook", "surface", "computer"],
    electronics: ["laptop", "tablet", "headphone", "monitor", "camera", "console"],
    phone: ["iphone", "phone", "android", "pixel", "samsung galaxy"],
  };

  const targetKeywords = [
    ...(category && keywordMap[category] ? keywordMap[category] : []),
    ...keywords,
  ]
  ...
  const totalSpent = items.reduce((sum, itm) => sum + (itm.total || 0), 0);
...
["total_spend", "fsa_spend", "recent_orders", "top_products",
 ...
 "budget_recommendation", "debt_analysis", "monthly_breakdown", "shopping_recommendation", "purchase_review", "fallback"]
...
Q: "I want to buy a new pair of shoes, help me decide" or "Recommend me some electronics" or "I want to get a meal from doordash"
-> {"intent":"shopping_recommendation","time_range":{"start":null,"end":null},"limit":null,"params":{"category":"shoes"}}

Q: "I want to buy a new laptop, but I'm worried I'm spending too much"
-> {"intent":"purchase_review","time_range":{"start":null,"end":null},"limit":null,"params":{"category":"laptop"}}
`;
    ...
    if (lowerMessage.includes("help me decide") || lowerMessage.includes("recommend") || 
        (lowerMessage.includes("want to buy") || lowerMessage.includes("want to get"))) {
      // Extract category if mentioned
      let category = null;
      if (lowerMessage.includes("shoe")) category = "shoes";
      else if (lowerMessage.includes("electronic") || lowerMessage.includes("device")) category = "electronics";
      else if (lowerMessage.includes("meal") || lowerMessage.includes("food") || lowerMessage.includes("doordash") || lowerMessage.includes("uber")) category = "food";
...
app.post("/cortex", async (req, res) => {
  ...
  try {
    ...
    const rawAnswer = await formatResultsWithChatGPT(
      result.results,
      query.trim(),
      result.query
    );
    const answer = formatFinalMessage(rawAnswer);
    const chunks = splitMessageIntoChunks(answer, CHAT_CHUNK_SIZE);
    ...
    return res.json({
      ...
      answer,
      answerChunks: chunks,
      chunkDelayMs: CHAT_CHUNK_DELAY_MS
    });
  } catch (err) {
    console.error("[ERROR] Error processing Cortex query:", err);
    const fallbackAnswer =
      "Sorry, I'm having trouble processing your query right now. Please try again.";
    const chunks = splitMessageIntoChunks(fallbackAnswer, CHAT_CHUNK_SIZE);
...
app.post("/chat", async (req, res) => {
  ...
  try {
    ...
    const finalAnswer = formatFinalMessage(answer);
    const chunks = splitMessageIntoChunks(finalAnswer, CHAT_CHUNK_SIZE);
...
async function formatResultsWithChatGPT(results, userQuery, sqlQuery) {
  if (USE_DEDALUS_BRIDGE) {
    try {
      const bridgeResponse = await callDedalusBridge("/format-results", {
      ...
      if (bridgeResponse?.formatted) {
        return bridgeResponse.formatted;
      }
    } catch (error) {
      console.warn("[DEDALUS] Falling back from bridge format:", error.message);
    }
  }
